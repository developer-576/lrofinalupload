...
try{
  ...
  $fileId=(int)($_POST['file_id']??0); if($fileId<=0) throw new Exception('Missing file_id');
  $reason=trim((string)($_POST['reason_text']??'')); $adminId=(int)($_SESSION['admin_id']??0);

  $tblNoPrefix = 'lrofileupload_uploads';
  $tblPref     = _DB_PREFIX_.$tblNoPrefix;
  ...
  $data=[
    ...
    'rejection_notes'=>($reason!==''?pSQL($reason,true):null),
  ];
  ...
  echo json_encode(['ok'=>true,'file_id'=>$fileId,'status'=>'rejected']);
}catch(Throwable $e){
